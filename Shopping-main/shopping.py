...
def load_data(filename):
    ...
    df['Weekend'] = df['Weekend']*1 # change bool to 0/1
    df['Revenue'] = df['Revenue']*1 # change bool to 0/1
    ...
    evidence = list(list(x) for x in zip(*(df[x].values.tolist() for x in df.columns)))
